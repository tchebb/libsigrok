...
static void send_samples(const struct sr_dev_inst *sdi, unsigned char *buf, size_t len)
{
	...
	for (size_t i = 0; i < len; i += devc->in_ep_pkt_size) {
		...
		pkt_len -= NUM_STATUS_BYTES;
		if (pkt_len == 0) {
			sr_info("Received empty data packet");
			continue;
		}

		if (devc->limit_samples && pkt_len > remaining)
			pkt_len = remaining;
		...
		sr_spew("Sending %zu samples.", pkt_len);
		sr_session_send(sdi, &packet);

		devc->samples_sent += pkt_len;
		if (devc->limit_samples) {
			remaining -= pkt_len;
			if (remaining == 0) {
				sr_info("Requested number of samples reached.");
				ftdi_la_stop_acquisition(sdi);
				break;
			}
		}
	}
...
SR_PRIV int ftdi_la_stop_acquisition(const struct sr_dev_inst *sdi)
{
	struct dev_context *devc = sdi->priv;
